...
%I do not use option pdfusetitle (which must be introduced here and not in hypersetup) in articles because the title often has some asterisk or other supplement that must not appear in the pdf metadata. TODO For presentations, Beamer implicitely uses the pdfusetitle option. ntheorem doc says to load hyperref “before the first use of \newtheorem”. autonum doc mandates option hypertexnames=false.
%	\usepackage[hypertexnames=false, pdfusetitle]{hyperref}
%TODO the beamer version
	\usepackage{hyperref}
	%I hide links for those which can be recognized as links by the reader without highlighting, to not distract the reader. urlbordercolor is used both for \url (and \doi) and for \href, thus might need to be removed. TODO test in presentation. If using colorlinks=true in a presentation, see http://tex.stackexchange.com/questions/203056/changing-url-colors-in-headline-footline-of-beamer-template.
...
%TODO {\mathversion{bold} formula} seems equivalent to \boldsymbol{formula}? Not at my place!
%TODO \mathbf{v} and \boldsymbol{v} differ: the first one is upright, the second one is italic.
	\usepackage{bm}
%https://ctan.org/pkg/amsmath recommends ntheorem, which supersedes amsthm, which corrects the spacing of proclamations and allows for theoremstyle. Option standard loads amssymb and latexsym. Must be loaded after amsmath (from ntheorem doc).
%TODO define LCpres auto depending on class
	\nottoggle{LCpres}{
...
%Provides \cref. Unfortunately, cref fails when the language is French and referring to a label whose name contains a colon (https://tex.stackexchange.com/questions/83798/cleveref-varioref-missing-endcsname-inserted). Use \cref{sec\string:intro} to work around. cleveref should go “laster” than hyperref.
	\usepackage{cleveref}
%Equations get numbers iff they are referenced. Loading order should be “amsmath → hyperref → cleveref → autonum”, according to autonum doc. Use this in preference to the showonlyrefs option from mathtools, see https://tex.stackexchange.com/q/459918 and autonum doc. See https://tex.stackexchange.com/questions/285950/package-autonum-needs-the-obsolete-etex-package for the etex line, TODO.
...
%Provides \print­length{length}, useful for debugging.
	%\usepackage{printlen}
	%\uselengthunit{mm}
%Provides \NewDocumentCommand and similar commands possibly intended as replacement of \newcommand in LaTeX3 (for package authors? see https://tex.stackexchange.com/questions/98152/always-use-newdocumentcommand-instead-of-newcommand and https://github.com/latex3/latex2e/issues/89).
	%\usepackage{xparse}
